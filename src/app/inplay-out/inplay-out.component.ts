...
import { SportsListService } from "./inplay-out.service";
...
@Component({
  selector: "app-home",
  templateUrl: "./inplay-out.component.html",
  styleUrls: ["./inplay-out.component.css"],
})
export class InplayOutComponent implements OnInit, AfterViewInit, OnDestroy {
  IsEmail = null;
  loginTypeOtp;
  ...
  getseiresMatchsList() {
    var sdata = { limit: 100, pageno: 1, sport_id: 0, series_id: 0 };
    this.sportsListService.getseiresMatchsList(sdata).subscribe(
      (data) => {
        ...
        if (!data.error) {
          ...
          //console.log(result);
          if (!isNullOrUndefined(result)) {
            this.matchesListInplay = result.InplayMatches;
          }
        }
  ...
  CheckSportExist(sId) {
    var isExist = false;
    if (this.matchesListInplay != undefined) {
      for (var i = 0; i < this.matchesListInplay.length; i++) {
        if (this.matchesListInplay[i].sport_id == sId) {
          isExist = true;
          break;
        }
      }
    ...
    this.loginservice.getbalance().subscribe(
      (response) => {
        if (!response.error) {
          ...
          //this.router.navigate(['/dashboard']);
          //window.location.href = 'https://localhost:4200/dashboard';
          window.location.href = "https://" + environment.domain + "/dashboard";
        }
      },
      (error) => {
        this.getBalance();
