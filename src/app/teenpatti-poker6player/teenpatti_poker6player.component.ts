...
import { TeenpattiPoker6PlayerService } from './teenpatti_poker6player.services';
...
@Component({
  ...
  templateUrl: './teenpatti_poker6player.component.html',
  styleUrls: ['./teenpatti_poker6player.component.css']
})
export class TeenpattiPoker6PlayerComponent implements OnInit, OnDestroy {
  ...
  constructor(public _sessionService: SessionService,
    public _teenpattiService: TeenpattiPoker6PlayerService,
  ...
  GetMarketList() {
    try {
      this.detailApiTimer = setTimeout(() => {
        if (this.route.url == '/poker-6player') {
  ...
  getPlayerAdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 0) {
      ...
      if (playerA.cards.length < 2) {
        var extraCardNeed = 2 - playerA.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
  ...
  getPlayerBdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 1) {
      ...
      if (playerB.cards.length < 2) {
        var extraCardNeed = 2 - playerB.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
  ...
  playerDData: any = null;
  getPlayerDdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 3) {
      var playerA = this.matchDetail.runner_json[3];
      if (playerA.cards.length < 2) {
        var extraCardNeed = 2 - playerA.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
          playerA.cards.push('0');
      ...
      this.playerDData = playerA;
      return;
    }
    this.playerDData = null;
  }

  playerEData: any = null;
  getPlayerEdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 4) {
      var playerA = this.matchDetail.runner_json[4];
      if (playerA.cards.length < 2) {
        var extraCardNeed = 2 - playerA.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
          playerA.cards.push('0');
      ...
      this.playerEData = playerA;
      return;
    }
    this.playerEData = null;
  }

  playerFData: any = null;
  getPlayerFdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 5) {
      var playerA = this.matchDetail.runner_json[5];
      if (playerA.cards.length < 2) {
        var extraCardNeed = 2 - playerA.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
          playerA.cards.push('0');
      ...
      this.playerFData = playerA;
      return;
    }
    this.playerFData = null;
  }
  ...
  updateBetBox() {
    ...
    if (playerData == null && this.playerDData != null) {
      if (this.betBoxPlayerData.id == this.playerDData.id) {
        playerData = this.playerDData;
      }
    }
    if (playerData == null && this.playerEData != null) {
      if (this.betBoxPlayerData.id == this.playerEData.id) {
        playerData = this.playerEData;
      }
    }
    if (playerData == null && this.playerFData != null) {
      if (this.betBoxPlayerData.id == this.playerFData.id) {
        playerData = this.playerFData;
      }
    }
    ...
    if (this.betBoxPlayerData.odds <= 0) {
      ...
      this._sportService.saveCasinoBetData(sdata).subscribe(data => {
        ...
        if (!data.error) {
          ...
          if (!isNullOrUndefined(this.playerDData)) {
            this.playerCData.WinAndLoss = this.getSelectionPL(this.playerCData);
          }

          if (!isNullOrUndefined(this.playerEData)) {
            this.playerEData.WinAndLoss = this.getSelectionPL(this.playerEData);
          }

          if (!isNullOrUndefined(this.playerFData)) {
            this.playerFData.WinAndLoss = this.getSelectionPL(this.playerFData);
          }
  ...
  openMatchResult(result) {
    ...
    if(this.selectedResult!=null){
      ...
      if(this.selectedResult.runners.length>4){
        this.selectedResult.playerE=this.selectedResult.runners[4];
      }else{
        this.selectedResult.playerE=null;
      }

      if(this.selectedResult.runners.length>5){
        this.selectedResult.playerF=this.selectedResult.runners[5];
      }else{
        this.selectedResult.playerF=null;
      }
    }
  }
