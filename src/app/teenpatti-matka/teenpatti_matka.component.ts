...
import { TeenpattiMatkaService } from './teenpatti_matka.services';
...
@Component({
  ...
  templateUrl: './teenpatti_matka.component.html',
  styleUrls: ['./teenpatti_matka.component.css']
})
export class TeenpattiMatkaComponent implements OnInit, OnDestroy {
  ...
  constructor(public _sessionService: SessionService,
    public _teenpattiService: TeenpattiMatkaService,
  ...
  updateOldMatchDetailData(matchDetailData) {
    try {
      if (matchDetailData != null) {
        ...
        if (this.matchDetail.indexCard.length < 1) {
          var extraCardNeed = 1 - this.matchDetail.indexCard.length;
  ...
  updateCountDownTimerData(currentTimerupdate) {
    ...
    if (this.last_ui_timer != currentTimer) {
      ...
      var perfixPreviousTime = "";
      if (previousTime.length < 4) {
        var extraNeed = 4 - previousTime.length;
        for (let index = 0; index < extraNeed; index++) {
          perfixPreviousTime += "0";
        }
      }

      var perfixCurrentTime = "";
      if (currentTimer.length < 4) {
        var extraNeed = 4 - currentTimer.length;
        for (let index = 0; index < extraNeed; index++) {
          perfixCurrentTime += "0";
        }
      }

      previousTime = perfixPreviousTime + previousTime;
      currentTimer = perfixCurrentTime + currentTimer;

      var last_ui_timer_0 = (previousTime).charAt(0);
      var last_ui_timer_1 = (previousTime).charAt(1);
      var last_ui_timer_2 = (previousTime).charAt(2);
      var last_ui_timer_3 = (previousTime).charAt(3);

      var currentTimer_0 = (currentTimer).charAt(0);
      var currentTimer_1 = (currentTimer).charAt(1);
      var currentTimer_2 = (currentTimer).charAt(2);
      var currentTimer_3 = (currentTimer).charAt(3);
      ...
      if (last_ui_timer_2 != currentTimer_2) {
        this.timer_pos_2 = false;
        change_2 = true;
      }

      if (last_ui_timer_3 != currentTimer_3) {
        this.timer_pos_3 = false;
        change_3 = true;
      }
      ...
      setTimeout(() => {
        ...
        if (change_2) {
          this.timer_pos_2 = true;
        }

        if (change_3) {
          this.timer_pos_3 = true;
        }
  ...
  getPlayerAdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 0) {
      ...
      for (let pos = 0; pos < this.matchDetail.runner_json.length; pos++) {
        var playerA = this.matchDetail.runner_json[pos];
  ...
  updatePL() {
    ...
    if (parseFloat(this.betBoxPlayerData.odds) >= 0) {
      this.betBoxPlayerData.p_l = ((parseFloat(this.betBoxPlayerData.odds) * this.betBoxPlayerData.stack));
    }
  }
  ...
  getCurrentBetboxPL() {
    ...
    if (parseFloat(this.betBoxPlayerData.odds) >= 0) {
      return ((parseFloat(this.betBoxPlayerData.odds) * this.betBoxPlayerData.stack));
    }
  ...
  updateBetBox() {
    ...
    if (this.allPlayersData != null) {

      for (let pos = 0; pos < this.allPlayersData.length; pos++) {
        var playerA = this.allPlayersData[pos];
        if (this.betBoxPlayerData.id == playerA.id) {
          playerData = playerA;
          break;
        }
      }
    ...
    if (this.betBoxPlayerData.odds <= 0) {
      ...
      this._sportService.saveCasinoBetData(sdata).subscribe(data => {
        ...
        if (!data.error) {
          ...
          if(this.allPlayersData!=null){
            for (let pos = 0; pos < this.allPlayersData.length; pos++) {
              var element = this.allPlayersData[pos];
              element.WinAndLoss=this.getSelectionPL(element);
            }
          }
