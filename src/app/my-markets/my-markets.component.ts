...
export class MyMarketsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._sportService.GetMyMarketsMatchList(0)
  ...
  CheckSportExist(sId) {
    ...
    var isExist = false;
    if (this._sportService._ListMyMarketsMatch != undefined) {
      for (var i = 0; i < this._sportService._ListMyMarketsMatch.length; i++) {
        if (this._sportService._ListMyMarketsMatch[i].sport_id == sId) {
          isExist = true;
          break;
        }
      }
