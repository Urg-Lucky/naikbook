...
declare const $: any;
@Component({
  selector: 'app-setbutton-values',
  templateUrl: './setbutton-values.component.html',
  styleUrls: ['./setbutton-values.component.css']
})
export class SetbuttonValuesComponent implements OnInit {
  ...
  constructor(
    ...
    public _SetbuttonValuesService: SetbuttonValuesService,
  ...
  getDWStatement() {
    ...
    this._SetbuttonValuesService.getDWRequestStatement(data).subscribe(data => {
      if (!data.error) {
        if (this.page == 1) {
  ...
  CancelRequest = function () {
    ...
    if (this.cancel_form.invalid) {
      ...
      this._SetbuttonValuesService.cancelRequest(data).subscribe(response => {
