...
import { Teenpatti32CardsService } from './teenpatti_32cards.services';
...
@Component({
  ...
  templateUrl: './teenpatti_32cards.component.html',
  styleUrls: ['./teenpatti_32cards.component.css']
})
export class Teenpatti32CardsComponent implements OnInit, OnDestroy {
  ...
  constructor(public _sessionService: SessionService,
    public _teenpattiService: Teenpatti32CardsService,
  ...
  updateOldMatchDetailData(matchDetailData) {
    try {
      if (matchDetailData != null) {
        ...
        this.getPlayerAdata();
        this.getPlayerAPlusdata();
        this.getPlayerBdata();
        this.getPlayerBPlusdata();
        this.updateBetBox();
      }
    }
    catch (e) {
  ...
  getNumberFromString(value){
    ...
    for (let index = 0; index < data.length; index++) {
      ...
      try{
        finalNumber+= isNaN(parseInt(element))?'':parseInt(element).toString();
      }catch{
  ...
  getPlayerAdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 0) {
      ...
      if (playerA.cards.length < 1) {
        var extraCardNeed = 1 - playerA.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
          playerA.cards.push('0');
        }
      }
      var cardTotalNumber=0;
      playerA.cards.forEach(element => {
        cardTotalNumber+= this.getNumberFromString(element);
      });
      if(cardTotalNumber>0){
        cardTotalNumber+=8;
      }
      playerA.cardTotalNumber=cardTotalNumber;
  ...
  getPlayerAPlusdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 2) {
      ...
      this.playerAPlusData = playerA;
      return;
    }
    this.playerAPlusData = null;
  }
  ...
  getPlayerBdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 1) {
      ...
      if (playerB.cards.length < 1) {
        var extraCardNeed = 1 - playerB.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
          playerB.cards.push('0');
        }
      }

      var cardTotalNumber=0;
      playerB.cards.forEach(element => {
        cardTotalNumber+= this.getNumberFromString(element);
      });
      if(cardTotalNumber>0){
        cardTotalNumber+=9;
      }
      playerB.cardTotalNumber=cardTotalNumber;
  ...
  playerBPlusData: any = null;
  getPlayerBPlusdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 3) {
      var playerB = this.matchDetail.runner_json[3];
      if (playerB.cards.length < 1) {
        var extraCardNeed = 1 - playerB.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
          playerB.cards.push('0');
      ...
      this.playerBPlusData = playerB;
      return;
    }
    this.playerBPlusData = null;
  }
  ...
  updateBetBox() {
    ...
    if (playerData == null && this.playerAPlusData != null) {
      if (this.betBoxPlayerData.id == this.playerAPlusData.id) {
        playerData = this.playerAPlusData;
      }
    }
    if (playerData == null && this.playerBPlusData != null) {
      if (this.betBoxPlayerData.id == this.playerBPlusData.id) {
        playerData = this.playerBPlusData;
      }
    }
    ...
    if (this.betBoxPlayerData.odds <= 0) {
      ...
      this._sportService.saveCasinoBetData(sdata).subscribe(data => {
        ...
        if (!data.error) {
          ...
          if(!isNullOrUndefined(this.playerAPlusData)){
            this.playerAPlusData.WinAndLoss = this.getSelectionPL(this.playerAPlusData);
          }

          if(!isNullOrUndefined(this.playerBPlusData)){
            this.playerBPlusData.WinAndLoss = this.getSelectionPL(this.playerBPlusData);
          }
  ...
  openMatchResult(result) {
    ...
    if(this.selectedResult!=null){
      if(this.selectedResult.runners.length>0){
        ...
        var cardTotalNumber=0;
        this.selectedResult.playerA.cards.forEach(element => {
          cardTotalNumber+= this.getNumberFromString(element);
        });
        if(cardTotalNumber>0){
          cardTotalNumber+=8;
        }
        this.selectedResult.playerA.cardTotalNumber=cardTotalNumber;
      ...
      if(this.selectedResult.runners.length>1){
        ...
        var cardTotalNumber=0;
        this.selectedResult.playerB.cards.forEach(element => {
          cardTotalNumber+= this.getNumberFromString(element);
        });
        if(cardTotalNumber>0){
          cardTotalNumber+=9;
        }
        this.selectedResult.playerB.cardTotalNumber=cardTotalNumber;
      }else{
      ...
      if(this.selectedResult.runners.length>2){
        ...
        var cardTotalNumber=0;
        this.selectedResult.playerC.cards.forEach(element => {
          cardTotalNumber+= this.getNumberFromString(element);
        });
        if(cardTotalNumber>0){
          cardTotalNumber+=10;
        }
        this.selectedResult.playerC.cardTotalNumber=cardTotalNumber;
      }else{
        this.selectedResult.playerC=null;
      }

      if(this.selectedResult.runners.length>3){
        this.selectedResult.playerD=this.selectedResult.runners[3];

        var cardTotalNumber=0;
        this.selectedResult.playerD.cards.forEach(element => {
          cardTotalNumber+= this.getNumberFromString(element);
        });
        if(cardTotalNumber>0){
          cardTotalNumber+=11;
        }
        this.selectedResult.playerD.cardTotalNumber=cardTotalNumber;
      }else{
        this.selectedResult.playerD=null;
      }
    }
  }
