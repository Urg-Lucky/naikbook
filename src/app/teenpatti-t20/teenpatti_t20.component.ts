...
import { TeenpattiT20Service } from './teenpatti_t20.services';
...
@Component({
  ...
  templateUrl: './teenpatti_t20.component.html',
  styleUrls: ['./teenpatti_t20.component.css']
})
export class TeenpattiT20Component implements OnInit, OnDestroy {
  ...
  constructor(public _sessionService: SessionService,
    public _teenpattiService: TeenpattiT20Service,
  ...
  GetMarketList() {
    try {
      this.detailApiTimer = setTimeout(() => {
        if (this.route.url == '/teenpatti-t20') {
                ...
                if (!isNullOrUndefined(matchDetailData.match_id)) {
                  ...
                  if (this.callType == 1) {
                    ...
                    if (this.matchDetail == undefined) {
                      ...
                      this.getMatchBetAndFancyBetList();
                      this.getForceLast10Result();
                      if (this.safeSrc != this.sanitizer.bypassSecurityTrustResourceUrl(matchDetailData.MainTV)) {
                        if (matchDetailData.MainTV != '') {
                          this.safeSrc = this.sanitizer.bypassSecurityTrustResourceUrl(matchDetailData.MainTV);
                        }
                      }
  ...
  getPlayerAdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 0) {
      ...
      if (playerA.cards.length < 3) {
        var extraCardNeed = 3 - playerA.cards.length;
  ...
  getPlayerBdata() {
    if (this.matchDetail != null && this.matchDetail.runner_json.length > 1) {
      ...
      if (playerB.cards.length < 3) {
        var extraCardNeed = 3 - playerB.cards.length;
        for (let index = 0; index < extraCardNeed; index++) {
