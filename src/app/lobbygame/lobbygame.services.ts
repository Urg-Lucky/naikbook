...
export class LobbygameService {
  ...
  getGameLobbyStart(sdata) {
    let httpParams = new HttpParams();
    Object.keys(sdata).forEach(function (key) {
      httpParams = httpParams.append(key, sdata[key]);
    });
    return this.http.post<any>(this.endpoint2 + 'game-login', sdata, { headers: this._sessionService.setTokenHeader() });
  }
  ...
  getGameLobby(sdata) {
    ...
    return this.http.post<any>(this.endpoint + 'game-start', sdata, { headers: this._sessionService.setTokenHeader() });
  }
