...
import { TeenpattiDragonTigerService } from './teenpatti_dragontiger.services';
...
@Component({
  ...
  templateUrl: './teenpatti_dragontiger.component.html',
  styleUrls: ['./teenpatti_dragontiger.component.css']
})
export class TeenpattiDragonTigerComponent implements OnInit, OnDestroy {
  ...
  constructor(public _sessionService: SessionService,
    public _teenpattiService: TeenpattiDragonTigerService,
  ...
  GetMarketList() {
    try {
      this.detailApiTimer = setTimeout(() => {
        if (this.route.url == '/dragon-tiger') {
