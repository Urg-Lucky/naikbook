...
export class CasinoSlotsAuthService {
  ...
  getCasinoLobbyByMerchantID(sdata) {
    let httpParams = new HttpParams();
    Object.keys(sdata).forEach(function (key) {
      httpParams = httpParams.append(key, sdata[key]);
    });
    return this.http.post<any>(this.endpoint + 'lobby', sdata, { headers: this._sessionService.setTokenHeader() });
  }
