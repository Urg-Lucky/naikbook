...
export class DetailsComponent implements OnInit {
  ...
  loading = false;
  _serverTime: any;
  horseBetMatchMarkets: any = [];
  horseBetUserSportsDetails: any = [];
  ...
  constructor(
    ...
    public _horseService: HorseServiceService,
  ...
  getMatchScoreBySportAndMatchId() {
    ...
    if (
      ...
      this.matchScore = setTimeout(
        () => {
          if (this.route.url == "/horse-details") {
            var sdata = {
      ...
      this.matchForSoccer = setTimeout(
        () => {
          if (this.route.url == "/horse-details") {
            ...
            this._horseService.GetHorseMarketList(sdata).subscribe(
              (data) => {
                if (!data.error) {
                  ...
                  this.horseBetUserSportsDetails = result.UserSportSettings[0];
                  this.matchStack =
                    this.horseBetUserSportsDetails.match_stack.split(",");
                  this.UserOneClickStack =
                    this.horseBetUserSportsDetails.one_click_stack.split(",");
                  if (result.MatchDetails != null) {
                    ...
                    if (this.callType == 1) {
                      this.horseBetMatchMarkets = result.MatchDetails;
                      // this.horseBetUserSportsDetails=result.UserSportSettings[0];
                      // this.UserOneClickStack=this.horseBetUserSportsDetails.one_click_stack.split(',');
                      this.setcupBetMatchData(result.MatchDetails);
                      if (this.horseBetMatchMarkets.runner_json != null) {
                        for (
                          let i = 0;
                          i < this.horseBetMatchMarkets.runner_json.length;
                          i++
                        ) {
                          this._sportService.AssignKeyInit(
                            this.horseBetMatchMarkets.runner_json[i]
                              .selectionId,
                            this.horseBetMatchMarkets.match_id,
                            this.horseBetMatchMarkets.market_id
                          );
                        }
                      }
                    } else {
                      if (this.horseBetMatchMarkets == undefined) {
                        this.horseBetMatchMarkets = result.MatchDetails;
                      } else if (
                        this.horseBetMatchMarkets.market_id !=
  ...
  setcupBetMatchData(result) {
    try {
      if (result != null) {
        ...
        this.horseBetMatchMarkets.matchVolumn = cupBetMatch.matchVolumn;
        this.horseBetMatchMarkets.BetAllowTimeBefore =
          cupBetMatch.BetAllowTimeBefore;
        this.horseBetMatchMarkets.InplayStatus = cupBetMatch.InplayStatus;
        this.horseBetMatchMarkets.IsBetAllow = cupBetMatch.IsBetAllow;
        this.horseBetMatchMarkets.SportName = cupBetMatch.SportName;
        this.horseBetMatchMarkets.SportmaxOddsLimt =
          cupBetMatch.SportmaxOddsLimt;
        this.horseBetMatchMarkets.SportminOddsLimt =
          cupBetMatch.SportminOddsLimt;
        this.horseBetMatchMarkets.backRateDiff = cupBetMatch.backRateDiff;
        this.horseBetMatchMarkets.favMatchID = cupBetMatch.favMatchID;
        this.horseBetMatchMarkets.layRateDiff = cupBetMatch.layRateDiff;
        this.horseBetMatchMarkets.sportGraphic = cupBetMatch.sportGraphic;
        this.horseBetMatchMarkets.sportScore = cupBetMatch.sportScore;
        this.horseBetMatchMarkets.sportShowLastResult =
          cupBetMatch.sportShowLastResult;
        this.horseBetMatchMarkets.sportShowTV = cupBetMatch.sportShowTV;
        this.horseBetMatchMarkets.start_date = cupBetMatch.start_date;
        this.horseBetMatchMarkets.adminMessage = cupBetMatch.adminMessage;

        var btBefor = this.horseBetMatchMarkets.BetAllowTimeBefore;
        if (this.horseBetMatchMarkets.start_date - btBefor > this._serverTime) {
          this.horseBetMatchMarkets.isDetail = false;
          var timeStamp = this._sportService.timeDifference(
            (this.horseBetMatchMarkets.start_date - btBefor) * 1000,
            this._serverTime * 1000
          );

          this.horseBetMatchMarkets.remainTime = timeStamp;
        } else {
          this.horseBetMatchMarkets.isDetail = true;
          this.horseBetMatchMarkets.remainTime = "Inplay";
        }
        if (
          ...
          for (let i = 0; i < cupBetMatch.runner_json.length; i++) {
            var indx = this.horseBetMatchMarkets.runner_json.findIndex(
              (x) => x.selectionId == cupBetMatch.runner_json[i].selectionId
            );
            if (indx > -1) {
              this.horseBetMatchMarkets.runner_json[indx].WinAndLoss =
                cupBetMatch.runner_json[i].WinAndLoss;
              if (
                this.horseBetMatchMarkets.runner_json[indx].ex
                  .availableToBack != undefined &&
                this.horseBetMatchMarkets.runner_json[indx].ex.availableToBack
                  .length > 0
              ) {
                for (
                  var r = 0;
                  ...
                  try {
                    ...
                    if (newBackBhav.price + "" != oldBackBhav.price + "") {
                      ...
                      this.horseBetMatchMarkets.runner_json[
                        indx
                    ...
                    // if (this.horseBetMatchMarkets.runner_json[indx].ex.availableToBack[r] == undefined) {
                    this.horseBetMatchMarkets.runner_json[
                      indx
                    ].ex.availableToBack[r].price = "--";
                    // }
              ...
              if (
                this.horseBetMatchMarkets.runner_json[indx].ex.availableToLay !=
                  undefined &&
                this.horseBetMatchMarkets.runner_json[indx].ex.availableToLay
                  .length > 0
              ) {
                for (
                  ...
                  try {
                    ...
                    // if(this.horseBetMatchMarkets.runner_json[indx].ex.availableToLay[r]==undefined){
                    this.horseBetMatchMarkets.runner_json[
                      indx
                    ].ex.availableToLay[r].price = "--";
                    // }
  ...
  setBetBox(type, soccerrunners) {
    ...
    soccerrunners.market_id = this.horseBetMatchMarkets.market_id;
    soccerrunners.match_id = this.horseBetMatchMarkets.match_id;
        ...
        (data) => {
          if (!data.error) {
            ...
            if (betfair == 0) {
              for (
                var r = 0;
                r < this.horseBetMatchMarkets.runner_json.length;
                r++
              ) {
                this.horseBetMatchMarkets.runner_json[r].WinAndLoss =
                  this.horseBetMatchMarkets.runner_json[r].WinAndLoss +
                  this._sportService.stake2[
                    "field_" +
                      this.horseBetMatchMarkets.runner_json[r].selectionId +
                      "_" +
                      this.horseBetMatchMarkets.match_id +
                      "_" +
                      this.horseBetMatchMarkets.market_id
